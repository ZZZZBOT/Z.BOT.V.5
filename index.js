...
autoketik = false;
prefixStatus = false;
targetpc = "94764122693"; 
owner = "94764122693"; 
fakeyoi = "IMASH NIDUSHA(ZED)"; 
HunterApi = "Ikyy69", 
xchillds = 'XChillDs' 
hardi = 'hardianto', 
valkey = "rivalgans", 
...
nomorowner1 = "@94764122693"; 
fake = `ᴏᴡɴᴇʀ : ${fakeyoi}\n🦊 ᴘʀɪꜰɪx :「 ${prefixStatus ? "Multi Prefix" : "No Prefix"} 」`;
...
const checkRegisteredUser = (sender) => {
    ...
    const ownerNumber = [`94764122693@s.whatsapp.net`]
...
const daftar1 = `Hi ${pushname} ${ucapanWaktu} \n\nBefore Using the Bot Verify First `
       const daftar2 = '*┃YOUR NUMBER IS NOT VERIFYD┃*\nclick the button below to verify bro or type 8473\n\nPOWERD BY Z BOT```'
       const daftar3 = [
          {
            buttonId: `verify`,
            buttonText: {
...
/*const reply = (teks) => {
			xeon.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/94764122693?text=hello`}}})
		}*/
...
xeon.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
    ...
    const add = function (from, orangnya) {
      xeon.groupAdd(from, orangnya);
        ...
        const grupinv = (teks) => {
        	grup = xeon.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/Dgt6JhzTvlmEor8Zz23fHx', "groupName": `Xeon Bot Inc.`, "footerText": "*_ZED BOT_*", "jpegThumbnail": iye, "caption": teks}}, {quoted:mek})
...
const fakeitem = (teks) => {
        ...
        participant:`0@s.whatsapp.net`, ...(from ? {
remoteJid :"6289523258649-1604595598@g.us" }: {})
                    },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync(`./stik/thumb.jpeg`),"itemCount":10,"status":"INQUIRY","surface":"CATALOG","message":`ZED BOT`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true})}
// Katalog
const reply = (teks) => {
...
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Xeon`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/94764122693?text=hello`}}})
		}
...
sendEphemeral: false,
"externalAdReply": {
                "title": `Whatsapp-BOT\nZ BOT. (@nnda.krnwn_) •Instagram photos and Videos`,
                "body": "",
                "previewType": "PHOTO",
...
"externalAdReply": {
                "title": `Whatsapp`,
                "body": `wa.me/94764122693`,
                "mediaType": "10",
                "mediaUrl": `wa.me/94764122693`,
                "thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
                "thumbnail": iye,
                "sourceUrl": `wa.me/94764122693`,
...
async function sendFileFromUrl(from, url, caption, msg, men) {
    ...
    if (isGroup && !isVote) {
      if (budy.toLowerCase() === "vote") {
        ...
        let id_vote = sender ? sender : "94764122693@s.whatsapp.net";
...
*Thank you for registering in the ZED BOT Database*
...
*「 ZED BOT 」*`
	    ...
	    // console.log(e)
			...
			reply(`*Thank you for being registered ZED BOT*`)
		}, 2000)
        break
...
groups = xeon.chats.array.filter(v => v.jid.endsWith('g.us'))
              ...
              total = math(`${groups.length}*${privat.length}`)
if (!isRegistered) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: fkontak})
        ptod = "94764122693@s.whatsapp.net"
        dtod = '94764122693'
        stod = `${sender}`
        uwuu = '```'
        yoii = '*'
...
case 'bugreport':
              ...
              xeon.sendMessage('94764122693@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
...
case 'script':
		case 'sc':
		case 'sourcecode':
		xeon.sendMessage(from, { text: "wa.me/94764122693", matchedText: 'nopee', description: "", title: "What are you doing?", jpegThumbnail: iye }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
      //------------------< Sticker Cmd >-------------------
      case "addcmd":
      case "setcmd":
...
sendButMessage(from, tiyo, `ZED BOT`, [
            {              
   ...
   case 'bc':
      case 'broadcast':
        if (!mek.key.fromMe) return;
             ...
             for (let _ of anu) {
             xeon.sendMessage(_.jid, bc, image, {quoted:fkontak,caption: `*「 ZED BOT BROADCASTING 」*\n\n${body.slice(4)}`})
}
